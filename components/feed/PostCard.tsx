...
export function PostCard({
  ...
  const [editedAt, setEditedAt] = useState<string | null>(
    post.editedAt || null
  );
  const [pinOrder, setPinOrder] = useState<number | null>(
    post.pinOrder || null
  );
  ...
  const [displayIdentity, setDisplayIdentity] = useState<PostIdentity>(() => {
    if (post.postedAsType === "faith_admin") {
      ...
      const orgAvatar =
        avatarCache?.organizations.get((post as any).postedAsOrgId) ||
        post.avatarUrl ||
        null;
      return {
        type: "organization",
  ...
  const [postEventId, setPostEventId] = useState<string | null>(
    eventId || null
  );
  ...
  const loadPostData = async () => {
    try {
      ...
      const { data: postData } = await supabase
        .from("posts")
        .select(
          "posted_as_type, posted_as_org_id, event_id, comments, edited_at, pin_order, reaction_count, repost_count, author_id"
        )
        .eq("id", post.id)
        .single();
      ...
      if (postData.posted_as_type === "user") {
        ...
        postData.posted_as_type === "organization" &&
        postData.posted_as_org_id
      ) {
        // Use cached avatar instead of fetching
        const orgAvatar =
          avatarCache?.organizations.get(postData.posted_as_org_id) || null;
        ...
        <div className="p-6">
          ...
          <div className="flex items-start gap-3 mb-4">
            ...
            <button
              ...
              className={
                isAuthorClickable ? "cursor-pointer" : "cursor-default"
              }
            >
              {displayIdentity.avatarUrl ? (
                ...
                <div
                  className={`h-12 w-12 rounded-xl bg-gradient-to-br ${getAuthorColor(
                    displayIdentity.type
