            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {data.depts.map((name) => (
                  <span
                    key={name}
                    className="px-3 py-1 bg-emerald-50 text-emerald-700 text-sm rounded-lg border border-emerald-100 font-medium"
                  >
                    {name}
                  </span>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {data.courses.map((name) => (
                  <span
                    key={name}
                    className="px-3 py-1 bg-purple-50 text-purple-700 text-sm rounded-lg border border-purple-100 font-medium"
                  >
                    {name}
                  </span>
                ))}
    ...
    <>
      <div className="flex flex-wrap gap-1.5 items-center mt-1">
        {allItems.slice(0, PREVIEW_LIMIT).map((item, i) => (
          <span
            key={i}
            className={`text-[10px] px-2 py-0.5 rounded-full font-bold truncate max-w-[120px] ${item.color}`}
          >
            {item.name}
          </span>
        ))}
...
function EventDetailsContent() {
  ...
  const loadEventData = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        if (orgRes.data)
          setCurrentUserOrgs(orgRes.data.map((o: any) => o.organization_id));
      }
    ...
    try {
      ...
      const mappedPosts = postsData.map((post: any) => {
        ...
        if (post.posted_as_type === "faith_admin") {
          ...
          post.posted_as_type === "organization" &&
          post.posted_as_org_id
        ) {
          displayName = orgNameMap.get(post.posted_as_org_id) || "Organization";
          displayAvatar =
            avatarCache.organizations.get(post.posted_as_org_id) || null;
  ...
  const canEdit =
    isFaithAdmin ||
    (event.organizerType === "organization" &&
      event.creatorOrgId &&
      currentUserOrgs.includes(event.creatorOrgId)) ||
  ...
  return (
    <>
      <div className="max-w-5xl mx-auto py-8 px-4">
        <button
           onClick={() => router.push('/home?tab=events')}
        ...
        <div
          className={`rounded-2xl border-2 overflow-hidden mb-6 transition-colors relative ${
        ...
        >
          ...
          <div className="p-8">
            ...
            {event.description && (
              <p className="text-gray-700 text-lg mb-6 leading-relaxed whitespace-pre-wrap">
                {event.description}
              </p>
            )}
            ...
            {event.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {event.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 px-3 py-1 bg-white/60 rounded-full text-xs font-bold text-gray-700 border border-gray-200"
                  >
                  ...
                  <div
                    ...
                    className={`transition-all duration-500 ${
                      highlightedId === post.id
                        ? "ring-4 ring-blue-400 rounded-xl"
                        : ""
                    }`}
                  >
                    <PostCard
