// app/(site)/edit-bulletin/[id]/page.tsx
"use client"
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        // Pre-fill form
        setHeader(bulletinData.header)
        setBody(bulletinData.body)
        setCreatorType(bulletinData.creator_type)
        setSelectedCreatorOrg(bulletinData.creator_org_id || '')
        setExistingImageUrls(bulletinData.image_urls || [])
        setAudienceType(bulletinData.audience_type)
        setAudOrgs(bulletinData.audience_orgs || [])
        setAudDepts(bulletinData.audience_depts || [])
        ...
        alert('Failed to load bulletin data')
        router.push('/home')
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-black text-gray-900 mb-2">Edit Bulletin Post</h1>
        <p className="text-gray-600">Update your bulletin information</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        {(isFaithAdmin || userOrgs.length > 0) && (
          <div className="bg-white rounded-xl border-2 border-gray-200 p-6">
            <label className="block text-sm font-bold text-gray-900 mb-3">Creator:</label>
            <div className="space-y-2">
              {isFaithAdmin && (
                <label className="flex items-center gap-3 p-4 border-2 border-purple-200 rounded-lg cursor-not-allowed opacity-75">
                  <input type="radio" checked={creatorType === 'faith_admin'} disabled className="w-4 h-4" />
              ...
              {userOrgs.map(org => (
                <label key={org.id} className="flex items-center gap-3 p-4 border-2 border-orange-200 rounded-lg cursor-not-allowed opacity-75">
                  <input type="radio" checked={creatorType === 'organization' && selectedCreatorOrg === org.id} disabled className="w-4 h-4" />
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-6 space-y-4">
                ...
                {existingImageUrls.map((url, idx) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      type="button"
                      className="absolute top-2 right-2 p-1.5 bg-red-500 text-white rounded-full hover:bg-red-600 shadow-lg opacity-0 group-hover:opacity-100 transition-all"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
          ...
          {totalImageCount < MAX_IMAGES && (
            ...
            >
              ...
              <span>Add More Images ({totalImageCount}/{MAX_IMAGES})</span>
            </button>
          )}
        </div>
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="flex-1 py-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl font-bold flex justify-center gap-2">
            {isSubmitting ? <Loader2 className="animate-spin" /> : <><Save className="h-5 w-5" /> Update Bulletin</>}
          </button>
        </div>
      </div>
    </div>
